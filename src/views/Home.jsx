import axios from "axios";
import { useContext, useState } from "react";
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import { faSpinner } from "@fortawesome/free-solid-svg-icons";
import { DataContext } from "../context/DataProvider";

const Home = () => {
    const [search, setSearch] = useState(null)
    const { user, setUser } = useContext(DataContext);



    const getData = async () => {
        const resp = await axios.get("http://localhost:5000/major-cities");
        return resp.status === 200 ? resp.data : null
    }

    const loadData = async () => {
        let data = await getData()
            .then(data => handleData(data))
    }

    const handleData = (data) => {
        console.log(data);
        setCities(data)
    }

    const [cities, setCities] = useState(() => loadData());

    const toggleTime = (feed, key) => {
        let time12 = document.getElementById(`time12-${key}`)
        let time24 = document.getElementById(`time24-${key}`)
        let toggle12 = document.getElementById(`toggle12-${key}`)
        let toggle24 = document.getElementById(`toggle24-${key}`)
        if (feed === "twelve") {
            time12.classList.remove('d-none')
            time24.classList.add('d-none')
            toggle24.classList.add('faded')
            toggle12.classList.remove('faded')
        } else if (feed === "twentyfour") {
            time24.classList.remove('d-none')
            time12.classList.add('d-none')
            toggle12.classList.add('faded')
            toggle24.classList.remove('faded')
        }
    }

    const updateSearch = (e) => {
        setSearch(e.target.value)
    }

    const searchCity = () => {
        if (!search) {
            alert("Please enter a city")
        } else {
            let data = search
            const resp = axios.post('http://localhost:5000/get-city', JSON.stringify(data), {
                headers : {"Content-Type": "application/json"}
            })
            .then(resp => handleSearch(resp)) 
        }
    }

    const handleSearch = (resp) => {
        console.log(resp.data)
        if (resp.data.message === "noCityFound") {
            alert("City not found. Try checking your spelling")
        } else if (resp.data.message === "cityFound") {
            console.log(resp.data.cities)
            setCities(resp.data.cities)
        }
    }

    const addCity = async (city_name) => {
        let data = {"user_id": user.id, "city_name": city_name}
        const resp = await axios.post('http://localhost:5000/add-city', JSON.stringify(data), {
            headers : {"Content-Type": "application/json"}
        })
        .then(resp => handleCity(resp))
    }

    const removeCity = async (city_name) => {
        let data = {"user_id": user.id, "city_name": city_name}
        const resp = await axios.post('http://localhost:5000/remove-city', JSON.stringify(data), {
            headers : {"Content-Type": "application/json"}
        })
        .then(resp => handleCity(resp))
    }

    const handleCity = (resp) => {
        console.log(resp.data)
        setUser(resp.data.data)
    }


    return (
        <>
            <h1 className="center-text mt-10">ClockCity</h1>
            {/* <div className="card-table flx-r just-se">
                <div className="time-card flx-c">
                    <div className="five flx-1">
                        <p className="city">City</p>
                        <p className="weather">weather</p>
                        </div>
                    <div className="six flx-3">
                        <div className="flx-r">
                            <div className="flx-c flx-5">
                                <p className="time">00:00 AM</p>
                                <p className="day">Tuesday 15th August</p>
                            </div>
                            <div className="nine format-toggle flx-1 pt-5">
                                <div className="center-text">24hr</div>
                                <div className="center-text">12hr</div>
                            </div>
                        </div>
                    </div>
                    <div className="seven flx-1"><p className="timezone">Timezone</p></div>
                </div>
            </div> */}


            {/* <div className="empty"></div> */}
            <div className="search-bar-div rel center wide80">
                <input id="search-bar" onChange={(e) => updateSearch(e)} className="center px-2" type="text" placeholder="Enter city name"></input>
                <img onClick={() => searchCity()} className="mag-glass-img abs" src="/images/mag-glass-search-bar-big-light.png" />
            </div>
            
            <div className="card-table flx-r flx-wrap just-se">
                {Array.isArray(cities) ? cities.map((c, i) => {
                    return <div key={i} className="time-card2 flx-c rel mx-3 my-4">
                        {user.saved.indexOf(c.city.toLowerCase()) > -1 ? null : <img onClick={() => addCity(c.city)} className="add-img abs" src="/images/plus-black-blue.png" />}
                        <div className=" flx-1">
                            <p className="city">{c.city}</p>
                            <p className="weather">weather</p>
                        </div>
                        <div className=" flx-3">
                            <div className="flx-r">
                                <div className="flx-c flx-5">
                                    <p id={`time12-${i}`} className="time2">{c.time12}</p>
                                    <p id={`time24-${i}`} className="time2 d-none">{c.time24}</p>
                                    <p className="day">{c.date_formatted}</p>
                                </div>
                                <div className=" format-toggle flx-1 pt-3h">
                                    <div id={`toggle24-${i}`} onClick={() => toggleTime("twentyfour", i)} className="center-text faded">24hr</div>
                                    <hr className="wide80" />
                                    <div id={`toggle12-${i}`} onClick={() => toggleTime("twelve", i)} className="center-text">12hr</div>
                                </div>
                            </div>
                        </div>
                        <div className=" flx-1"><p className="timezone gray-text small">Timezone: {c.timezone}</p></div>
                    </div>


                }) : <h3><FontAwesomeIcon icon={faSpinner} spin />&nbsp;&nbsp;Loading</h3>}

            </div>
        </>
    )
}
export default Home;